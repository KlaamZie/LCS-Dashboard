...
export default function EditDetail({ data, id }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setMessage('Détail modifié !');
    setDisplay(true);

    await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/detail?id=${id}`, input);
  ...
  const handleAddValue = async (e) => {
    e.preventDefault();
    await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/detail/value?id=${id}`, { value: newValue.current.value })
      .then((res) => {
        const value = res.data.detail.values.slice(-1);
        setInput({
  ...
  const handleDeleteValue = async (e, valueId) => {
    e.preventDefault();
    await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/detail/value?id=${id}`, { data: { valueId } })
      .then(() => {
