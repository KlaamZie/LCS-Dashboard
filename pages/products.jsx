...
export default function Products() {
  ...
  const { productsData, isLoading, isError } = useProducts(page);

  const handleCopy = async (el) => {
    const newProducts = productsData.products.slice();
    newProducts.push({
      ...el, _id: `${el._id}1`, name: `${el.name} (copy)`, state: false,
    });
  ...
  const handleDelete = async (id) => {
    const newProducts = productsData.products.slice();
    newProducts.splice(newProducts.findIndex((c) => c._id === id), 1);
  ...
  return (
    <Layout title="Products">
      <section>
        ...
        />
        <Pagination page={page} setPage={setPage} data={productsData} />
      </section>
    </Layout>
  );
}
