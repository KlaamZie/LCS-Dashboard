...
const InputImages = ({ input, setInput }) => {
  const handleFiles = async (e) => {
    ...
    for (let i = 0; i < files.length && input.images.length + i + 1 <= 5; i++) {
      formData.append('images', files[i]);
    }

    await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/upload-img-multiple`, formData)
      .then((res) => {
