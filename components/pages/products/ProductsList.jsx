import useProducts from '@/hooks/useProducts';

import FindCategory from '@/components/pages/utils/FindCategory';

import styles from '@/styles/modules/ItemsTable.module.scss';

import PropTypes from 'prop-types';

import Link from 'next/link';

const ProductsList = ({
  page, handleCopy, handleDelete,
}) => {
  const { productsData } = useProducts(page);

  return (
    <table className={styles.table}>
      <thead>
        <tr>
          <th>Nom</th>
          <th>Prix</th>
          <th>Categorie</th>
          <th>Status</th>
          <th>Actions</th>
        </tr>
      </thead>
      <tbody>
        {productsData.products.map((el) => (
          <tr key={el._id}>
            <td>{el.name}</td>
            <td>
              {el.price.toFixed(2)}
              {' '}
              €
            </td>
            <td><FindCategory category={el.mainCategory} /></td>
            <td>{ el.state ? 'Actif' : 'Inactif'}</td>
            <td>
              <Link href={`/product/edit/${el._id}`}>
                <a>
                  <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M11.8536 1.14645C11.6583 0.951184 11.3417 0.951184 11.1465 1.14645L3.71455 8.57836C3.62459 8.66832 3.55263 8.77461 3.50251 8.89155L2.04044 12.303C1.9599 12.491 2.00189 12.709 2.14646 12.8536C2.29103 12.9981 2.50905 13.0401 2.69697 12.9596L6.10847 11.4975C6.2254 11.4474 6.3317 11.3754 6.42166 11.2855L13.8536 3.85355C14.0488 3.65829 14.0488 3.34171 13.8536 3.14645L11.8536 1.14645ZM4.42166 9.28547L11.5 2.20711L12.7929 3.5L5.71455 10.5784L4.21924 11.2192L3.78081 10.7808L4.42166 9.28547Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd" /></svg>
                </a>
              </Link>
              <svg onClick={() => handleCopy(el)} width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 2V1H10V2H5ZM4.75 0C4.33579 0 4 0.335786 4 0.75V1H3.5C2.67157 1 2 1.67157 2 2.5V12.5C2 13.3284 2.67157 14 3.5 14H11.5C12.3284 14 13 13.3284 13 12.5V2.5C13 1.67157 12.3284 1 11.5 1H11V0.75C11 0.335786 10.6642 0 10.25 0H4.75ZM11 2V2.25C11 2.66421 10.6642 3 10.25 3H4.75C4.33579 3 4 2.66421 4 2.25V2H3.5C3.22386 2 3 2.22386 3 2.5V12.5C3 12.7761 3.22386 13 3.5 13H11.5C11.7761 13 12 12.7761 12 12.5V2.5C12 2.22386 11.7761 2 11.5 2H11Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd" /></svg>
              <svg onClick={() => handleDelete(el._id)} width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.5 1C5.22386 1 5 1.22386 5 1.5C5 1.77614 5.22386 2 5.5 2H9.5C9.77614 2 10 1.77614 10 1.5C10 1.22386 9.77614 1 9.5 1H5.5ZM3 3.5C3 3.22386 3.22386 3 3.5 3H5H10H11.5C11.7761 3 12 3.22386 12 3.5C12 3.77614 11.7761 4 11.5 4H11V12C11 12.5523 10.5523 13 10 13H5C4.44772 13 4 12.5523 4 12V4L3.5 4C3.22386 4 3 3.77614 3 3.5ZM5 4H10V12H5V4Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd" /></svg>
            </td>
          </tr>
        ))}
      </tbody>
    </table>
  );
};

ProductsList.propTypes = {
  page: PropTypes.number.isRequired,
  handleCopy: PropTypes.func.isRequired,
  handleDelete: PropTypes.func.isRequired,
};

export default ProductsList;
